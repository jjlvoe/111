...
def plot_chart(chart_type, df):
    chart_dict = {
        ...
        '饼图': px.pie,
        '散点图': px.scatter,
        '雷达图': lambda df: create_radar_chart(df),
        '箱型图': px.box,
        '树形图': px.treemap
    }
    fig = chart_dict[chart_type](df, x='Word', y='Count') if chart_type != '雷达图' else chart_dict[chart_type](df)
...
if url:
    ...
    if text is None:
        ...
        df_word_counts = pd.DataFrame(filtered_word_counts.items(), columns=['Word', 'Count'])
        chart_type = st.sidebar.selectbox('选择图表类型', ['条形图', '折线图', '饼图', '散点图', '雷达图', '箱型图', '树形图'])
